...
@Component({
  ...
  template: `
    ...
    <!-- Error message -->
    <div *ngIf="errorMsg" class="mt-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-600 text-sm">
      ⚠️ {{ errorMsg }}
    </div>
    ...
    <style>
      ...
      .stat-label { font-size: 0.78rem; opacity: 0.85; letter-spacing: 0.05em; text-transform: uppercase; }
      .stat-value { font-size: 2.2rem; font-weight: 800; line-height: 1.1; }
      .stat-value.loading { opacity: 0.5; }
      .stat-sub { font-size: 0.75rem; opacity: 0.75; margin-top: 2px; }
    </style>
  `
})
